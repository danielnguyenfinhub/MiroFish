...
from action_logger import PlatformActionLogger, SimulationLogManager  # noqa: E402
    ...
    async def handle_interview(
        ...
        if platform in ("twitter", "reddit"):
            ...
            if "error" in result:
                ...
                print(
                    f"  Interview失败: agent_id={agent_id}, platform={platform}, error={result['error']}"
                )
                return False
            else:
        ...
        if success_count > 0:
            ...
            print(
                f"  Interview完成: agent_id={agent_id}, 成功平台数={success_count}/{len(platforms_to_interview)}"
            )
            return True
        else:
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            await self.handle_interview(
                command_id, args.get("agent_id", 0), args.get("prompt", ""), args.get("platform")
            )
            return True
...
def _get_post_info(cursor, post_id: int, agent_names: dict[int, str]) -> dict[str, str] | None:
    """
    获取帖子信息
...
def _get_user_name(cursor, user_id: int, agent_names: dict[int, str]) -> str | None:
    """
    获取用户名称
...
def create_model(config: dict[str, Any], use_boost: bool = False):
    ...
    print(
        f"{config_label} model={llm_model}, base_url={llm_base_url[:40] if llm_base_url else '默认'}..."
    )
...
def get_active_agents_for_round(
    ...
    selected_ids = (
        random.sample(candidates, min(target_count, len(candidates))) if candidates else []
    )
...
async def run_twitter_simulation(
    config: dict[str, Any],
    simulation_dir: str,
    action_logger: PlatformActionLogger | None = None,
    main_logger: SimulationLogManager | None = None,
...
async def main():
    ...
    parser.add_argument(
        "--config", type=str, required=True, help="配置文件路径 (simulation_config.json)"
    )
    parser.add_argument("--twitter-only", action="store_true", help="只运行Twitter模拟")
    parser.add_argument("--reddit-only", action="store_true", help="只运行Reddit模拟")
    parser.add_argument(
        "--max-rounds", type=int, default=None, help="最大模拟轮数（可选，用于截断过长的模拟）"
    )
    parser.add_argument(
        "--no-wait",
        action="store_true",
    ...
    # 存储两个平台的模拟结果
    twitter_result: PlatformSimulation | None = None
    reddit_result: PlatformSimulation | None = None

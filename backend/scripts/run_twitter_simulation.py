...
class IPCHandler:
    ...
    async def process_commands(self) -> bool:
        ...
        if command_type == CommandType.INTERVIEW:
            await self.handle_interview(command_id, args.get("agent_id", 0), args.get("prompt", ""))
            return True
...
class TwitterSimulationRunner:
    ...
    async def run(self, max_rounds: int = None):
        ...
        for round_num in range(total_rounds):
            ...
            # 获取本轮激活的Agent
            active_agents = self._get_active_agents_for_round(self.env, simulated_hour, round_num)
...
async def main():
    ...
    runner = TwitterSimulationRunner(config_path=args.config, wait_for_commands=not args.no_wait)

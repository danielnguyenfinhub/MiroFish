...
def reset_project(project_id: str):
    ...
    return jsonify({"success": True, "message": t('api.projectReset', id=project_id), "data": project.to_dict()})
...
def generate_ontology():
    ...
    try:
        ...
        project.ontology = {"entity_types": ontology.get("entity_types", []), "edge_types": ontology.get("edge_types", [])}
...
def build_graph():
    ...
    try:
        ...
        if errors:
            return jsonify({"success": False, "error": t('api.configError', details="; ".join(errors))}), 500
        ...
        def build_task():
            ...
            try:
                task_manager.update_task(task_id, status=TaskStatus.PROCESSING, message=t('progress.initGraphService'))

...
def prepare_simulation():
    ...
    try:
        ...
        if not project:
            return jsonify(
                {"success": False, "error": t("api.projectNotFound", id=state.project_id)}
            ), 404
                ...
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify(
                                {"success": False, "error": t("api.simRunningForceHint")}
                            ), 400
    ...
    try:
        ...
        for i, interview in enumerate(interviews):
            if "agent_id" not in interview:
                return jsonify(
                    {"success": False, "error": t("api.interviewListMissingAgentId", index=i + 1)}
                ), 400
            if "prompt" not in interview:
                return jsonify(
                    {"success": False, "error": t("api.interviewListMissingPrompt", index=i + 1)}
                ), 400
            # 验证每项的platform（如果有）
            item_platform = interview.get("platform")
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify(
                    {"success": False, "error": t("api.interviewListInvalidPlatform", index=i + 1)}
                ), 400
        ...
        return jsonify(
            {"success": False, "error": t("api.batchInterviewTimeout", error=str(e))}
        ), 504
...
def interview_all_agents():
    ...
    try:
        ...
        return jsonify(
            {"success": False, "error": t("api.globalInterviewTimeout", error=str(e))}
        ), 504

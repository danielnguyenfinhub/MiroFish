...
def list_reports():
    ...
    try:
        ...
        return jsonify(
            {"success": True, "data": [r.to_dict() for r in reports], "count": len(reports)}
        )
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify(
                {"success": False, "error": t("api.reportProgressNotAvail", id=report_id)}
            ), 404
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        if not os.path.exists(section_path):
            return jsonify(
                {"success": False, "error": t("api.sectionNotFound", index=f"{section_index:02d}")}
            ), 404
